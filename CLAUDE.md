...
- **Visuels : PHOTO / vraies images uniquement. ⛔ PAS de SVG, PAS de monogrammes texte (« AN », « RB »).** Peter a explicitement refusé les SVG pour les couvertures (cartes featured `/games/`, cartes d'accueil). **Implémentation actuelle** : on réutilise l'image réelle `images/hero-bg.webp` (rendu gaming — Roblox à gauche, téléphone + créature à droite) recadrée par `object-position` : **Roblox → `left center`**, **Aniimo → `right center`**. Piloté par `coverImage` + `coverPosition` dans `data/games/<slug>.json`. Les anciens `images/cover-*.svg` ne sont plus référencés (à supprimer).
- **Pour ajouter une vraie photo dédiée à un jeu** : déposer un fichier bitmap (`.webp`/`.jpg`/`.png`) **libre de droit** dans `images/` (⚠️ jamais de hotlink ni de visuel officiel copyrighté), puis renseigner `coverImage`/`coverPosition` dans le JSON du jeu et régénérer (`python3 tools/build_site.py`). ⚠️ Mes outils ne peuvent PAS télécharger d'images binaires externes → c'est Peter qui fournit les photos, ou on réutilise un asset local existant / une miniature `tr.rbxcdn.com`.
